    ...
    <ModalCore isOpen={isOpen} handleClose={handleClose} position={EModalPosition.CENTER} width={EModalWidth.XL}>
      <div className="space-y-5 px-5 py-8 sm:p-6">
        <h3 className="text-16 leading-6 font-medium text-primary">
          {t("project_settings.members.join_confirmation.title")}
        </h3>
        <p>{t("project_settings.members.join_confirmation.description", { project: project?.name })}</p>
      ...
      <div className="mt-5 flex justify-end gap-2 px-5 pb-8 sm:px-6 sm:pb-6">
        ...
        <Button variant="primary" size="lg" tabIndex={1} type="submit" onClick={handleJoin} loading={isJoiningLoading}>
          {isJoiningLoading
            ? t("project_settings.members.join_confirmation.joining")
            : t("project_settings.members.join_confirmation.button")}
        </Button>
      </div>
    </ModalCore>
  );
